...
func NewPostgresListener(opts PostgresListenerOpts, handler glsql.ListenHandler) (*PostgresListener, error) {
	switch {
	...
	connectionLifecycle := func(eventType pq.ListenerEventType, err error) {
		...
		case pq.ListenerEventConnectionAttemptFailed:
			if firstConnectionAttempt {
				...
				// we should not proceed as it won't be possible to distinct between

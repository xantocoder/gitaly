...
func (imp *Importer) importVirtualStorage(ctx context.Context, virtualStorage string, output chan<- Result) error {
	...
	for {
		...
		if len(relativePaths) == batchSize {
			imported, err := imp.storeBatch(ctx, virtualStorage, shard.Primary.GetStorage(), relativePaths)
			if err != nil {
	...
	// store the final batch after the finisihing walking repositories
...
func (imp *Importer) storeBatch(ctx context.Context, virtualStorage, primary string, relativePaths []string) ([]string, error) {

...
func TestPerRepositoryRouter_RouteStorageAccessor(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			error:          ErrUnknownVirtualStorage,
		},
		{
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			router := NewPerRepositoryRouter(
				conns,
				nil,
				HealthCheckerFunc(func() map[string][]string {
...
func TestPerRepositoryRouter_RouteRepositoryAccessor(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			virtualStorage: "virtual-storage-1",
			healthyNodes: map[string][]string{
				"virtual-storage-1": {"primary", "consistent-secondary", "unconfigured-gitaly"},
			},
			numCandidates: 2,
			pickCandidate: 0,
			node:          "primary",
		...
		},
		{
			...
			virtualStorage: "virtual-storage-1",
			healthyNodes: map[string][]string{
				"virtual-storage-1": {"consistent-secondary", "unconfigured-gitaly"},
			},
			numCandidates: 1,
			node:          "consistent-secondary",
		},
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			router := NewPerRepositoryRouter(
				...
				HealthCheckerFunc(func() map[string][]string { return tc.healthyNodes }),
				RandomFunc(func(n int) int {
					t.Helper()
					require.Equal(t, tc.numCandidates, n)
					return tc.pickCandidate

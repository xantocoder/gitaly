...
func TestSuccessfulUserMergeToRefRequest(t *testing.T) {
	...
	commit, err := gitlog.GetCommit(ctx, testRepo, string(targetRef))
	require.NoError(t, err, "look up git commit after call has finished")
...
func TestSuccessfullyOverrideRefOnUserMergeToRefRequest(t *testing.T) {
	ctx, cancel := testhelper.Context()
	defer cancel()
	...
	out, err := exec.Command("git", "-C", testRepoPath, "update-ref", string(targetRef), beforeRefreshCommitSha).CombinedOutput()
	require.NoError(t, err, "give an existing state to the target ref: %s", out)

	beforeRefreshCommit, err := gitlog.GetCommit(ctx, testRepo, string(targetRef))
	require.NoError(t, err, "look up git commit before refresh ref call")
	require.Contains(t, beforeRefreshCommit.Id, beforeRefreshCommitSha, "target ref must have a state before receiving the refresh")
